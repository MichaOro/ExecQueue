...
if [[ -z "${DETACH_MODE:-}" ]]; then
    ...
    printf '[telegram_restart] Detaching from parent process (DETACH_MODE=1).\n' >> "${LOG_FILE}" 2>&1
    
    # Skript asynchron starten mit setsid
    setsid "${BASH_SOURCE[0]}" "$@" >> "${LOG_FILE}" 2>&1 &
    DETACHED_PID=$!
    
    printf '[telegram_restart] Restart script detached with PID %s.\n' "${DETACHED_PID}" >> "${LOG_FILE}" 2>&1
...
main() {
    ...
    log_to_file "Telegram bot restart completed successfully. (DETACH_MODE=${DETACH_MODE})"
    exit 0
}
