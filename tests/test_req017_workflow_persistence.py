...
class TestReq017SchemaAndEnums:
    ...
    def test_required_indexes_exist_on_models(self, db_session):
        ...
        assert "ix_task_executions_wf_task" in execution_indexes
...
class TestReq017Orchestrator:
    ...
    def test_idempotent_reuse_triggers_workflow_status_aggregation(self, db_session, monkeypatch):
        """When all tasks in a workflow are reused via idempotency, the workflow
        ...
        orchestrator._prepare_task_context(db_session, task, workflow.id)
    ...
    async def test_recovery_uses_completed_alias_cleanly(self, db_session):
        ...
        db_session.add(workflow)
        db_session.commit()
        create_task(db_session, workflow_id=workflow.id, task_number=1, status=TaskStatus.COMPLETED.value)
