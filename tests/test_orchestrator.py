...
def test_build_launch_plan_skips_bot_when_enabled_without_token():
    settings = Settings(telegram_bot_enabled=True, telegram_bot_token=None)

    plan = build_launch_plan(settings, python_executable="python")

    assert plan.bot_command is None
    assert plan.configuration_errors == (
        "Telegram bot is enabled but TELEGRAM_BOT_TOKEN is not set. Skipping bot startup.",
    )


def test_build_launch_plan_starts_bot_when_enabled_with_token():
    settings = Settings(telegram_bot_enabled=True, telegram_bot_token="test-token")
...
def test_monitor_processes_returns_zero_when_stop_signal_is_set():
    ...
    processes = [
        ...
        ManagedProcess(name="telegram-bot", process=FakePopen(pid=2, poll_result=1), required=False),
    ]
...
def test_orchestrator_ignores_legacy_acp_settings():
    ...
    assert plan.api_command[0] == "python"
    assert plan.bot_command is None
