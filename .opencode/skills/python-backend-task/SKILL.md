...
4. Check for downstream effects on orchestrator, runner, workflow, or worker code when the touched feature crosses boundaries.
5. Add or update pytest coverage in `tests/` for behavior changes.
6. Run relevant validation, usually `pytest`, before closing the task when feasible.

Checks:

- Do imports stay clean and local to the feature?
- Is the added abstraction justified by real reuse?
- Does the change keep the current codebase easier to understand without ignoring its existing multi-module complexity?
